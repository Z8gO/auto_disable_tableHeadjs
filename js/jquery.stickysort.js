	...
	$.fn.stickySort = function(opts) {
		...
		this.each(function() {
			if($(this).is('table') && $(this).find('thead').length > 0 && $(this).find('th').length > 0) {
				...
				//alert($t.find('thead th:first-child').attr("rowspan"));
				//var  fcrowspan=$t.find('thead th:first-child').attr("rowspan");
				//var  fccolspan=$t.find('thead th:first-child').attr("colspan");
				$stickyInsct.find('table').html('<thead><tr><th>'+$t.find('thead th:first-child').html()+'</th></tr></thead>');
				//$stickyInsct.find('table').html('<thead><tr><th rowspan='+fcrowspan+'  colspan='+fccolspan+'>'+$t.find('thead th:first-child').html()+'</th></tr></thead>');
				...
				var setWidths = function () {
						...
						/****************************↑↑↑↑↑修改为↓↓↓↓↓↓zhanghuang20190708**************************************/
						$stickyCol.find('thead th').add($stickyInsct.find('thead th')).width($t.find('thead th').first().width());
						
						//修改当设置表头rowspan 大于1时显示的bug. by  zhanghuang20190708
						$stickyInsct.find('th').first().height($t.find('thead th').first().height());
						/******************************************************************/
						/******************************************************************/
						/******************************************************************/
